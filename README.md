...
- 新的正常、困难或贪婪对局会根据当前角色、难度、未完成标记和开荒推荐等级，固定选择一条本局可完整兼容的宝箱层、暗室、Mother、祸兽/Home 或究极贪婪路线。路线 HUD 默认只显示去重后的本层操作，按住 `Tab` 才展开路线名称、成员、统一进度和下一步；发现普通下层活板门时会额外提醒尚未完成的本层必做事项，超级撒但路线也会在天使房和献祭房提示钥匙碎片。路线占一个追踪位但可同时覆盖多个成就，续局会恢复同一条路线。挑战、自定义种子、胜利跑圈和禁止解锁的对局不会生成推荐。
