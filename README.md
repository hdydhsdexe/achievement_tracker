...
- 新完成的成就会在当前房间以绿色逐条提示，进入下一房后消失；已追踪目标会自动取消，路线则移除已完成成员，但零成员的纯导航路线会保留至终点及已选可选 Boss 完成。F3 的已完成成就固定置底且不可再次追踪。
...
- 角色通关标记类目标会按当前楼层拆成“当前步骤 + 下一步”，逐层列出本层可进入的具体普通/支线楼层，覆盖 Mom's Heart、Isaac、Satan、Boss Rush、???、羔羊、超级撒但、究极贪婪、死寂、精神错乱、Mother 和祸兽路线；单项目标失败标红，追踪路线错过入口标黄，R Key 或合法替代方式可使路线重新恢复。
...
- 新的正常、困难或贪婪对局会先清空上局路线，并移除当前角色、模式、难度或路线条件下无法完成的普通追踪目标；开局已有可靠角色转化手段时保留对应目标。普通局会按成就推荐程度在母亲、羔羊、???、祸兽四个终点间推荐，贪婪与超级贪婪难度显示各自终点。路线 HUD 默认只显示实时本层操作；按住 `Tab` 才展开完整流程、成就成员和按“当前可用／仍可能取得／已失效”分类的补救措施。路线占一个追踪位且允许没有成就成员的纯导航，续局恢复同一路线。挑战、自定义种子、胜利跑圈和禁止解锁的对局不会生成推荐。
- 初始房间按 `V` 可一键追踪推荐路线；追踪路线后再次按 `V` 会追加一个与路线兼容的最高推荐普通目标。任意房间若存在尚未追踪的场景机会，`V` 总会优先追踪该机会；可靠的新入口出现时，随后按 `V` 可确认并延长当前路线。追踪栏已满时不会替换已有普通目标。
- `F3` 打开居中安全区域内的奖励可视化目标菜单：方向键或长按连续选择，鼠标悬停选择并可左键切换追踪，`Tab` 在全部、道具、饰品、卡牌、人物、怪物、地点、挑战、掉落物、机器与场景、机制、路线和其他奖励间循环筛选，`Enter`/`Space` 也可切换追踪，最多同时占用三个逻辑追踪位。路线行可统一追踪或取消整条路线，路线成员不可单独取消。取消普通目标时会留在当前位置，直到菜单因重新打开、搜索、筛选或状态变化而刷新。
- 在 F3 菜单按 `/` 可用英文、数字或缩写进行模糊搜索；搜索覆盖中英文成就名称、解锁奖励、完成条件、地点、所需角色/道具、奖励类型和编号，并可与 `Tab` 分类组合使用。
- `F3` 的“全部”和“路线”分类始终列出当前模式的全部终点路线，正在追踪者置顶，其余按四档成就推荐数量排序。“路线”分类会在当前聚焦终点下展开 Boss Rush、死寂、精神错乱以及羔羊/???专属的超级撒但子项；不兼容组合保留但灰显原因。详情显示带状态的完整楼层流程并支持 PageUp/PageDown 或详情区滚轮分页。路线可随时原位切换，错过入口时以黄色提示并等待可靠补救或新的现场入口。
- 安卡、碎安卡、犹大的影子、拉撒路的破布、寻人启事或遥控器只在已持有，或已发现且仍能回取时提前对应角色目标。当前层任意房间的来源均有效；留在先前宝箱房或头目房的来源仅为对应角色的祸兽单标记目标保留，回溯越过该层后自动失效。
- `F3` 在场景、本角色、转换角色和一般角色四组内按内置萌新开荒方案排列未追踪目标，并按“强烈推荐、推荐、普通、不建议提前解锁”显示推荐等级；强烈推荐、推荐和不建议提前解锁使用成就名称文字底色，普通等级无底色。手动追踪顺序固定，不可完成与已完成组保持成就目录顺序，搜索时仍以匹配度为先。
