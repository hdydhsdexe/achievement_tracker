...
- 新完成的成就会在当前房间以绿色逐条提示，进入下一房后消失；已追踪目标会自动取消，路线则移除已完成成员并在成员全部完成后结束。F3 的已完成成就固定置底且不可再次追踪。
...
- 新的正常、困难或贪婪对局会先清空上局路线，并移除当前角色、模式、难度或路线条件下无法完成的普通追踪目标；开局已有可靠角色转化手段时保留对应目标。随后根据当前角色、难度、未完成标记和开荒推荐等级，固定选择一条本局可完整兼容的宝箱层、暗室、Mother、祸兽/Home 或究极贪婪路线。路线 HUD 默认只显示实时本层操作：出口出现前提示击败本层头目，出现后列出当前普通与支线入口，不存在的入口以灰色显示；按住 `Tab` 才展开路线名称、成员、统一进度和完整下一步。发现普通下层活板门时会额外提醒尚未完成的本层必做事项，超级撒但路线也会在天使房和献祭房提示钥匙碎片。路线占一个追踪位但可同时覆盖多个成就，续局会恢复同一条路线。挑战、自定义种子、胜利跑圈和禁止解锁的对局不会生成推荐。
- 初始房间按 `V` 可一键追踪推荐路线；追踪路线后再次按 `V` 会追加一个与路线兼容的最高推荐普通目标。任意房间若存在尚未追踪的场景机会，`V` 总会优先追踪该机会。追踪栏已满时不会替换已有内容。
...
- `F3` 的“全部”和“路线”分类会列出本局所有仍有目标的路线：正在追踪的路线置顶，可进入或能由本层曾发现的关键物品恢复的路线按强烈推荐、推荐、普通、不建议目标数量排序，已错过的路线灰显置底。每条路线只占一个格子，详情显示四档数量、可解锁成员和首个失效原因；搜索可匹配路线名称、Boss 和全部成员。选择其他可达路线会原位替换当前路线，即使追踪栏已满也不额外占位；普通追踪目标不会被自动取消，若与新路线互斥，HUD 与 F3 会继续以红色提示冲突。
