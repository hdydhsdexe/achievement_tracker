...
- `F3` 会优先排列正在追踪的目标，再依次排列当前场景可立即完成、本角色专属、可转换角色、一般角色均可完成、当前条件不可完成、以及已完成的未追踪目标。角色不符、挑战模式不符或本局路线已经错过都会进入“当前条件不可完成”；其中未完成的挑战专属目标即使已追踪，在正常对局或其他挑战中仍留在该组。R Key 或合法路线恢复后会自动重新分组。
