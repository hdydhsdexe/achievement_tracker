...
function Menu.new() return { open=false, cursor=1, offset=1, goals=nil } end

local function refreshGoals(state)
  local pending, completed = {}, {}
  for _, goal in ipairs(Catalog.goals) do
    local done = (state.profileCompleted and state.profileCompleted[goal.id])
      or (state.run.completedGoals and state.run.completedGoals[goal.id])
    table.insert(done and completed or pending, goal)
  end
  for _, goal in ipairs(completed) do table.insert(pending, goal) end
  state.menu.goals = pending
end
...
function Menu.update(state, save)
  if triggered(Keyboard.KEY_F3) then
    state.menu.open = not state.menu.open
    if state.menu.open then refreshGoals(state) end
  end
  ...
  local goals = state.menu.goals or Catalog.goals
  local count = #goals
...
function Menu.render(state)
  ...
  y = y + lineHeight + 4
  local last = math.min(#goals, state.menu.offset + 10)
  for index = state.menu.offset, last do
    ...
    local completed = (state.profileCompleted and state.profileCompleted[goal.id])
      or (state.run.completedGoals and state.run.completedGoals[goal.id])
    local color = completed and KColor(0.25, 1, 0.35, 1) or KColor(1, 1, 1, 1)
    Text.draw(cursor .. tracked .. " " .. name, x, y, scale, color, language)
    y = y + lineHeight
  end
