...
- 新的正常、困难或贪婪对局会根据当前角色、难度、未完成标记和开荒推荐等级，固定选择一条本局可完整兼容的宝箱层、暗室、Mother、祸兽/Home 或究极贪婪路线。初始房间 HUD 会合并显示路线成员、统一进度及去重后的当前/下一步；路线占一个追踪位但可同时覆盖多个成就，续局会恢复同一条路线。挑战、自定义种子、胜利跑圈和禁止解锁的对局不会生成推荐。
