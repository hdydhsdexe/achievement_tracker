...
- 新的正常、困难或贪婪对局会根据当前角色、难度、未完成标记和开荒推荐等级，固定选择一条本局可完整兼容的宝箱层、暗室、Mother、祸兽/Home 或究极贪婪路线。路线 HUD 默认只显示实时本层操作：出口出现前提示击败本层头目，出现后列出当前普通与支线入口，不存在的入口以灰色显示；按住 `Tab` 才展开路线名称、成员、统一进度和完整下一步。发现普通下层活板门时会额外提醒尚未完成的本层必做事项，超级撒但路线也会在天使房和献祭房提示钥匙碎片。路线占一个追踪位但可同时覆盖多个成就，续局会恢复同一条路线。挑战、自定义种子、胜利跑圈和禁止解锁的对局不会生成推荐。
...
- `F3` 会优先排列正在追踪的普通目标及已追踪路线，再依次排列未追踪的推荐路线、当前场景可立即完成、本角色专属、可转换角色、一般角色均可完成、当前条件不可完成、以及已完成的未追踪目标。“全部”和“路线”分类会同时显示路线行及其成员；搜索可匹配路线名称、Boss 和成员成就。角色不符、挑战模式不符或本局路线已经错过都会进入“当前条件不可完成”；其中未完成的挑战专属目标即使已追踪，在正常对局或其他挑战中仍留在该组。R Key 或合法路线恢复后会自动重新分组。手动追踪与路线互斥的普通目标仍会保留，但 HUD 与 F3 会以红色提示冲突。
